...
export default function StreetFighter() {
  ...
  const startGame = (mode: GameMode, role: PlayerId = "player1") => {
    ...
    setWinner(null);
    setScreen("game");
    // Изчистване на клавишите и рестартиране на позициите
    resetKeys();
      ...
      <div className="min-h-screen bg-gray-900 flex flex-col items-center justify-center font-sans text-white p-4 bg-[url('https://www.transparenttextures.com/patterns/cubes.png')]">
        ...
        <div className="flex flex-col space-y-4 w-full max-w-sm">
          <button
            ...
            className="p-4 bg-linear-to-r from-blue-600 to-blue-800 rounded-xl font-bold text-xl shadow-lg hover:scale-105 transition-transform border border-blue-400"
          >
            🕹️ SINGLE PLAYER (VS AI)
          </button>
          <button
            onClick={() => startGame("local")}
            className="p-4 bg-linear-to-r from-green-600 to-green-800 rounded-xl font-bold text-xl shadow-lg hover:scale-105 transition-transform border border-green-400"
          >
            👥 LOCAL CO-OP
          </button>
          <button
            onClick={() => setScreen("lobby")}
            className="p-4 bg-linear-to-r from-purple-600 to-purple-800 rounded-xl font-bold text-xl shadow-lg hover:scale-105 transition-transform border border-purple-400"
          >
            🌐 ONLINE MULTIPLAYER
          </button>
        </div>
    ...
    <div className="min-h-screen bg-gray-900 flex flex-col items-center justify-center font-sans p-4 outline-none select-none">
      {gameMode === "online" && (
        <div className="absolute top-4 left-4 bg-gray-800 p-2 rounded border border-gray-700 text-xs text-gray-400">
          ...
          <span className="ml-2">
            | Ти си:{" "}
            {localPlayerRole === "player1" ? "P1 (Син)" : "P2 (Червен)"}
          </span>
        </div>
      )}
        ...
        {winner && (
          <div className="absolute inset-0 bg-black/80 flex flex-col items-center justify-center z-20 backdrop-blur-sm">
            ...
            <div className="flex gap-4">
              <button
                onClick={() => startGame(gameMode, localPlayerRole)}
                className="bg-red-600 hover:bg-red-500 text-white font-bold py-3 px-8 rounded-lg transition-transform hover:scale-105 shadow-lg"
              >
                РЕВАНШ
              </button>
              <button
      ...
      <div className="flex gap-12 mt-6 text-sm text-slate-400 bg-slate-800 p-4 rounded-xl shadow-lg border border-slate-700">
        <div>
          <p className="font-bold text-blue-400 mb-2 border-b border-slate-700 pb-1">
            ИГРАЧ 1 (СИН)
          </p>
          <p>
            <span className="text-white font-bold">W A S D</span> - Движение /
            Скок
          </p>
          <p>
            <span className="text-white font-bold">SPACE</span> - Удар
          </p>
        </div>
        <div>
          <p className="font-bold text-red-400 mb-2 border-b border-slate-700 pb-1">
            ИГРАЧ 2 (ЧЕРВЕН)
          </p>
          <p>
            <span className="text-white font-bold">СТРЕЛКИ</span> - Движение /
            Скок
          </p>
          <p>
            <span className="text-white font-bold">ENTER</span> - Удар
          </p>
        </div>
      </div>
    </div>
