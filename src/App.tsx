...
export default function StreetFighterPro() {
  ...
  const initConnection = (conn: any, role: PlayerId) => {
    connectionRef.current = conn;
    setConnection(conn);

    const startOnlineGame = () => {
      setStatus("Противникът се свърза! Зареждане...");
      startGame("online", role);
    };
    ...
    conn.on("error", (err: any) => {
      ...
      setStatus(`Грешка при връзка: ${err?.type || err?.message || err}`);
    });
    conn.on("close", () => {
      setStatus("Връзката беше прекъсната.");
    });
  ...
  const setupEntities = () => ({
    gameInfo: {
      ...
      conn: connectionRef.current || connection,
  ...
  if (screen === "menu") {
    return (
      <div className="min-h-screen bg-gray-900 flex flex-col items-center justify-center font-sans text-white p-4">
        ...
        <p className="text-gray-400 mb-12 tracking-widest uppercase">
          P2P Edition
        </p>
        ...
        <div className="flex flex-col space-y-4 w-full max-w-sm">
          <button
            ...
            className="p-4 bg-linear-to-r from-blue-600 to-blue-800 rounded-xl font-bold text-xl hover:scale-105 transition-transform border border-blue-400"
          >
            🕹️ SINGLE PLAYER (VS AI)
          </button>
          <button
            onClick={() => startGame("local")}
            className="p-4 bg-linear-to-r from-green-600 to-green-800 rounded-xl font-bold text-xl hover:scale-105 transition-transform border border-green-400"
          >
            👥 LOCAL CO-OP
          </button>
          <button
            ...
            className="p-4 bg-linear-to-r from-purple-600 to-purple-800 rounded-xl font-bold text-xl hover:scale-105 transition-transform border border-purple-400"
          >
            🌐 ONLINE MULTIPLAYER
          </button>
        </div>
  ...
  if (screen === "lobby") {
    return (
      <div className="min-h-screen bg-gray-900 flex flex-col items-center justify-center font-sans text-white p-4">
        ...
        {status && (
          <div className="mb-6 px-4 py-2 bg-yellow-900 text-yellow-200 border border-yellow-600 rounded-lg animate-pulse">
            {status}
          </div>
        )}
        ...
        <div className="bg-gray-800 p-8 rounded-xl border border-gray-700 w-full max-w-md shadow-2xl">
          <div className="mb-8 border-b border-gray-700 pb-6">
            ...
            <button
              onClick={createRoom}
              disabled={!!peerId}
              className="w-full p-3 bg-blue-600 rounded-lg font-bold hover:bg-blue-500 transition disabled:opacity-50"
            >
              Създай Стая
            </button>
            {peerId && (
              <div className="mt-4 p-3 bg-gray-900 rounded text-center">
                <p className="text-sm text-gray-400 mb-1">
                  Кажи този код на приятел:
                </p>
                <p className="font-mono text-xl text-green-400 font-bold select-all">
                  {peerId}
                </p>
              </div>
            )}
    ...
    <div className="min-h-screen bg-gray-900 flex flex-col items-center justify-center font-sans p-4 outline-none select-none">
      {gameMode === "online" && (
        <div className="absolute top-4 left-4 bg-gray-800 p-2 rounded border border-gray-700 text-xs text-gray-400">
          <span className="text-green-400">● P2P Свързан</span> | Ти си:{" "}
          {localPlayerRole === "player1" ? "ИГРАЧ 1 (Син)" : "ИГРАЧ 2 (Червен)"}
        </div>
      )}

      <div className="w-full max-w-4xl flex justify-between items-center mb-4 px-4 mt-8">
        <div className="w-1/3">
          <p className="text-blue-400 font-bold mb-1 text-lg">
            ИГРАЧ 1{" "}
            {localPlayerRole === "player1" && gameMode === "online" && "(ТИ)"}
          </p>
        ...
        <div className="text-4xl font-black text-yellow-500 italic drop-shadow-md">
          VS
        </div>
        ...
        <div className="w-1/3 text-right">
          <p className="text-red-400 font-bold mb-1 text-lg">
            {gameMode === "ai"
              ? "КОМПЮТЪР"
              : `ИГРАЧ 2 ${localPlayerRole === "player2" && gameMode === "online" ? "(ТИ)" : ""}`}
          </p>
          ...
          <div className="absolute inset-0 bg-black/80 flex flex-col items-center justify-center z-20 backdrop-blur-sm">
            ...
            <div className="flex gap-4">
              <button
                ...
                className="bg-gray-600 hover:bg-gray-500 text-white font-bold py-3 px-8 rounded-lg transition-transform hover:scale-105 shadow-lg"
              >
                КЪМ МЕНЮТО
              </button>
            </div>
